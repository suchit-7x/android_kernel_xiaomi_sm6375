...
static const char * const pctl_names[] = {
	"xiaomifp_reset_outputlow",
	"xiaomifp_reset_outputhigh",
	"xiaomifp_irq_active",
};
...
static int hw_reset(struct fpc1020_data *fpc1020)
{
	...
	rc = select_pin_ctl(fpc1020, "xiaomifp_reset_outputhigh");

	if (rc)
		goto exit;
	usleep_range(RESET_HIGH_SLEEP1_MIN_US, RESET_HIGH_SLEEP1_MAX_US);

	rc = select_pin_ctl(fpc1020, "xiaomifp_reset_outputlow");
	if (rc)
		goto exit;
...
static int device_prepare(struct fpc1020_data *fpc1020, bool enable)
{
	...
	if (enable && !fpc1020->prepared) {
		...
		select_pin_ctl(fpc1020, "xiaomifp_reset_outputlow");
		...
		(void)select_pin_ctl(fpc1020, "xiaomifp_reset_outputhigh");
	} else if (!enable && fpc1020->prepared) {
		rc = 0;
		(void)select_pin_ctl(fpc1020, "xiaomifp_reset_outputlow");
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(!strncmp(buf, "enable", strlen("enable")) && fpc1020->compatible_enabled != 1){
		rc = fpc1020_request_named_gpio(fpc1020, "fp-gpio-irq",
			&fpc1020->irq_gpio);
		if (rc)
			goto exit;

		rc = fpc1020_request_named_gpio(fpc1020, "fp-gpio-reset",
			&fpc1020->rst_gpio);
		...
		if (rc)
			goto exit;
		rc = select_pin_ctl(fpc1020, "xiaomifp_irq_active");
		if (rc)
			goto exit;
...
static struct of_device_id fpc1020_of_match[] = {
	{ .compatible = "xiaomi,fpcfingerprint", },
	{}
};
